...
void update_yuv_image_by_openh264_buffer_info(
  ...
  for (std::size_t i = 0; i < height0; ++i) {
    std::copy(buffer_info.pDst[0] + i * stride0,
              buffer_info.pDst[0] + i * stride0 + width0,
  ...
  for (std::size_t i = 0; i < height1; ++i) {
    std::copy(buffer_info.pDst[1] + i * stride1,
              buffer_info.pDst[1] + i * stride1 + width1,
              yuv_image->yuv[1] + i * width1);
    std::copy(buffer_info.pDst[2] + i * stride1,
              buffer_info.pDst[2] + i * stride1 + width1,
